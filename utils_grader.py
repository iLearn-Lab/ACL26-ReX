...
from utils_math_normalization import check_sympy_equivalence
...
def math_equal(
    ...
    for pattern in single_choice_patterns:
        if regex.match(pattern, prediction):
            ...
            if math_equal(prediction_cleaned, reference, include_percentage, is_close, timeout=timeout, depth=depth + 1,
                          max_depth=max_depth):
                return True
    ...
    if (
        ...
        if len(pred_parts) == len(ref_parts):
            if all(
                    [
                        math_equal(
                            pred_parts[i], ref_parts[i], include_percentage, is_close, timeout=timeout, depth=depth + 1,
                            max_depth=max_depth
                        )
    ...
    if prediction.count("=") == 1 and reference.count("=") == 1:
        ...
        if math_equal(
                prediction.split("=")[1], reference, include_percentage, is_close, timeout=timeout, depth=depth + 1,
                max_depth=max_depth
        ):
            return True
    elif (
        ...
        if math_equal(
                prediction, reference.split("=")[1], include_percentage, is_close, timeout=timeout, depth=depth + 1,
                max_depth=max_depth
        ):
            return True
